...
<script>
function follow(PU_followers_token_val){
	if('<?=$_SESSION[p_token_val]?>'){
		
		if('<?=$PU_idx?>'){
			var url = '/contents/member/bookmark?PU_idx=<?=$PU_idx?>';
		}else{
...
?>
				...
				<?}?>
					...
					<?if($rows_followers[token_val]==$_SESSION[p_token_val]){//자기글 조회?>
						<?if($following_ck){?>
							<a style="cursor:pointer;" class="btnUnFollow" onclick="follow('<?=$rows_followers[token_val]?>')">팔로우 취소</a>
						<?}else{?>
							<a style="cursor:pointer;" class="btnFollow" onclick="follow('<?=$rows_followers[token_val]?>')">팔로우</a>
						<?}?>
					<?}else{?>
					<?}?>

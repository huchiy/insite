...
// 순서변경
function activeBtn(u,PU_idx,Query) {
	var frm=document.ListForm;
	frm.Query.value = Query;
	frm.action=u+".php?PU_idx="+PU_idx;
	frm.submit();
}

// 순서변경
function delChg(u,PU_idx) {
...
// 좋아요
function petdol_like2(checkName,petdolbomi_idx) {
	$.ajax({
		type: 'POST',
		url: '../inc/ajax_check.php',
		data: {
			'checkName' : checkName,
			'petdolbomi_idx': petdolbomi_idx
		},
		cache: false,
		async: false,
		success: function(result) {
			...
			if(re[0]=="ok"){
				if(re[1]=='Y'){
					$("#heart_like_"+petdolbomi_idx).attr('src','/mimage/heart_on.png');
				}else{
					$("#heart_like_"+petdolbomi_idx).attr('src','/mimage/heart_off.png');
				}
			}else if(re[0]=="ok1"){
			}else {alert("오류가 있습니다");}
		}
...
function back(php_self){
	...
	if(php_self=='/contents/member/login.php' || php_self=='/contents/paykhan/paykhan.php' || php_self=='/contents/mypage/mypage.php' || php_self=='/contents/customer/customer.php'){ // 각탭 메인은 루트로
		location.replace('/');

	}else if(php_self=='/contents/mypage/privacy.php'){// 개인정보관리
		location.replace('/contents/mypage/mypage.php');
	}else if(php_self=='/contents/mypage/privacy_modify.php'){// 개인정보수정
		location.replace('/contents/mypage/privacy.php');

	}else if(php_self=='/contents/mypage/recommender.php'){// 커뮤니티
		location.replace('/contents/mypage/mypage.php');

	}else if(php_self=='/contents/customer/term_of_service.php'){// 이용약관
		location.replace('/contents/customer/customer.php');
	}else if(php_self=='/contents/customer/term_of_privacy.php'){// 개인정보취급방침
		location.replace('/contents/customer/customer.php');

	}else{
		location.replace('/contents/paykhan/paykhan.php');
	}

//	if ((escape(document.referrer).indexOf(php_self)!=-1) || (escape(document.referrer)=='') || (!(escape(document.referrer))) ) { //Find
//		if(php_self=='/contents/member/login.php' || php_self=='/contents/paykhan/paykhan.php' || php_self=='/contents/mypage/mypage.php' || php_self=='/contents/customer/customer.php'){ // 각탭 메인은 루트로
//			location.replace('/');
//		}else if(php_self=='/contents/mypage/privacy.php'){ //
//			location.replace('/contents/mypage/mypage.php');
//		}else if(php_self=='/contents/lounge.php'){ //
//			history.go(-1);
//		}else{
//			location.replace('/contents/paykhan/paykhan.php');
//		}

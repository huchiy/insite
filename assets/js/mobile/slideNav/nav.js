	...
	$.fn.panel = function(userConfig) {
		...
		// Config.
			config = $.extend({
					...
					//side: 'active', // Side of viewport the panel will appear.
			...
			if (config.resetScroll) {// 인태추가
				if($('#navPanel .active').length){ //active된 리스트가 있다면
					//$('#navPanel .active').css({"border":"1px solid red"});
					var wh = $(window).height();
					var wh2 = $(window).height() / 2;
					var lat = $('#navPanel .active').offset().top;
					if(lat >= wh - 25) {
						$this.scrollTop($('#navPanel .active').offset().top - wh2 + 15);
					}
				}
			}
					...
					// Post-hide stuff.
						window.setTimeout(function() {
							//if (config.resetScroll)
								//$this.scrollTop(0);
								//$this.scrollTop($('#navPanel .active').offset().top - 30);
